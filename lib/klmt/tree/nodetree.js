/*
    Keith Legg - Node Buddy in Webby places. 
	
          created "last saturday", August 13 , 2011
          modified                 August 20 , 2011
		  modified                 Sept   13 , 2011
...
//   throw "Error  : requires OpenLayers "; 
  ...
  /*
	...
	//GROK = new klmt.core.data_graph();
	//GROK.__init__('my graph');

	// EXAMPLE THAT USES BOTH CLASS ,NODE AND DATA TREE  // 
	
	NEWDAGCLASS = klmt.Class (klmt.core.data_graph,{});
	GROK = new NEWDAGCLASS();
	GROK.__init__('my graph');
	...
	//n3 =DAG.create('cc','foo');
	//GROK.create('n4','foo');
 
	
	//DAG.parent(n2,n1);
	GROK.parent_name('b','a');
	GROK.parent_name('c','a');
	GROK.parent_name('b','a');
	
	//alert(DAG.find_name('b') );
...
//klmt.core.data_graph = klmt.Class({ //klmt.NodeClass(
klmt.core.data_graph = klmt.Class ({
    ...
    walk : function (node){
      ...
      /***/
      //var walkchlds = node.get_children();
